...
#Plot predictions
plotPrediction(y_true, y_pred, 'Prediction of age', 'Size-adjusted features')

#%% Random forest - selected features (coefficient)

##Reshape X
#Define shapes
n_patients = 125 #number of patients
interpol_window = 1083 #number of windows interpolated
n_cluster = 1   #number of cluster-features (special-feature)
n_volume = 6    #number of volume-features (special-feature)
n_dist = 6      #number of dsitance-features (special-feature)
n_special = n_cluster + n_volume + n_dist
n_window = 210  #number of window-dependent features
n_features = n_window + n_special

#Extract window-independent features
X_special = X_flat[:,-n_special:]
X_window = X_flat[:,:-n_special] #delete special features

#Extract window-features
X_window = X_window.reshape(n_patients, interpol_window, n_window) #125 patients, 1083 inteprolated windows, n features)

##Look though coefficients - identify coefficients = 0
zero_coef_special = coef_special_df.columns[(coef_special_df == 0).any()] #special features with coefficient = 0
index_special = [coef_special_df.columns.get_loc(col) for col in zero_coef_special] #13 features with coefficient = 0

zero_coef_window = coef_mean_df.columns[(coef_mean_df == 0).any()] #window-dependent features with coefficient = 0
index_window = [coef_mean_df.columns.get_loc(col) for col in zero_coef_window] #50 features with coefficient = 0
n_window1 = n_window - len(index_window)

#Remove features with zero-coefficient from X_flat1
X_special1 = np.delete(X_special, index_special, axis=1) #no special features with coefficients ≠ 0
X_window1 = np.delete(X_window, index_window, axis=2) #160 window-dependent features with coefficients ≠ 0

##Flatten X_window and combine with special features into X_flat1
X_flat1 = X_window1.reshape(X_window1.shape[0], -1)
X_flat1 = np.hstack((X_flat1, X_special1))

##Random forest regression
y_true, y_pred, coef = ForestRegCV_func(X_flat1, y, n_splits = 5, param = best_param)

## Coefficients
#coef.shape # (227443,)
#Extract special-features 
#(there are no special features, since all have been removed during coef-filtering)

#Extract window-features
coef_mat = coef.reshape(1083, n_window1)
X_colnames1 = [col for col in X_colnames if col not in zero_coef_window]
coef_w_df = pd.DataFrame(coef_mat, columns = X_colnames1) #index = windows
coef_mean_df = pd.DataFrame(coef_w_df.mean(axis = 0)).T #mean over windows
coef_mean_df.columns = X_colnames1
coef_sum_df = pd.DataFrame(coef_w_df.sum(axis = 0)).T #sum over windows
coef_sum_df.columns = X_colnames1

#Save coefficients
path_file = os.path.join(path_root, '3.Model/Model_output/'+version+'/Coefficients_adjusted.xlsx')
os.makedirs(os.path.dirname(path_file), exist_ok=True)
#coef_df.to_excel(path_file, index=True)
with pd.ExcelWriter(path_file, engine='openpyxl') as writer:
    coef_w_df.to_excel(writer, sheet_name='Window-features', index=False)
    coef_mean_df.to_excel(writer, sheet_name='Mean-features', index=False)
    coef_sum_df.to_excel(writer, sheet_name='Sum-features', index=False)
...
##Plot predictions
plotPrediction(y_true, y_pred, 'Prediction of age', 'Size-adjusted and coef.-filtered features')
...
plotPrediction(y_true1, y_pred1, 'Prediction of age-group1', 'Size-adjusted features')
...
plotPrediction(y_true2, y_pred2, 'Prediction of age-group2', 'Size-adjusted features')
...
plotPrediction(y_true3, y_pred3, 'Prediction of age-group3', 'Size-adjusted features')
...
plt.title('Prediction of age in age-groups', fontsize = big, y = 1.05)
plt.suptitle('Size-adjusted features', fontsize = medium, y = 0.91, ha = 'center')
plt.scatter(y_true1, y_pred1, label = 'Age1: below 2 months', color = DTU_color[4])
plt.scatter(y_true2, y_pred2, label = 'Age2: between 2 and 4 months', color = DTU_color[3])
plt.scatter(y_true3, y_pred3, label = 'Age3: above 4 months', color = DTU_color[2])
plt.xlabel('True')
plt.ylabel('Pred')
plt.legend()
plt.xlim(min(y_true1 + y_true2 + y_true3), max(y_true1 + y_true2 + y_true3))
plt.ylim(min(y_true1 + y_true2 + y_true3), max(y_true1 + y_true2 + y_true3))
plt.show()
...
#Plot prediction
R2 = r2_score(y_true_ML, y_pred_ML)
R2_HO = r2_score(y_true_HO, y_pred_HO)

fig = plt.figure(figsize = (12, 9))
plt.grid(axis = 'y', alpha = 0.75)
plt.title('Prediction of age', fontsize = big, y = 1.05)
plt.suptitle('Size-adjusted features', fontsize = medium, y = 0.91, ha = 'center')

plt.scatter(y_true_ML, y_pred_ML, label = 'Training data', color = DTU_color[1])
plt.scatter(y_true_HO, y_pred_HO, label = 'Hold out data', color = DTU_color[0])

# Add tendency line (linear regression)
coefficients = np.polyfit(y_true_ML, y_pred_ML, 1)  # Linear fit (degree=1)
poly = np.poly1d(coefficients)
trendline = poly(y_true_ML)
plt.plot(y_true_ML, trendline, linestyle='-', linewidth=2, color = DTU_color[1])

coefficients = np.polyfit(y_true_HO, y_pred_HO, 1)  # Linear fit (degree=1)
poly = np.poly1d(coefficients)
trendline = poly(y_true_HO)
plt.plot(y_true_HO, trendline, linestyle='-', linewidth=4, color = DTU_color[0])

plt.text(0.735, 0.85, f'R² (training) = {R2:.4f}', transform=plt.gca().transAxes, fontsize=15, verticalalignment='top', 
         bbox=dict(facecolor='lightgrey', edgecolor='white', boxstyle='square, pad=0.4'))
plt.text(0.73, 0.79, f'R² (hold out) = {R2_HO:.4f}', transform=plt.gca().transAxes, fontsize=15, verticalalignment='top', 
         bbox=dict(facecolor='lightgrey', edgecolor='white', boxstyle='square, pad=0.4'))
plt.xlabel('True age [weeks]')
plt.ylabel('Predicted age [weeks]')
plt.legend()
plt.xlim(min(y_true_ML)-1, max(y_true_ML)+1)
plt.ylim(min(y_true_ML)-1, max(y_true_ML)+1)
